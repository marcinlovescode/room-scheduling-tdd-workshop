...
public class AvailableRoomsApiTests
{
    ...
    public async Task Returns_available_rooms()
    {
        ...
        var serializedQuery = JsonConvert.DeserializeObject<IDictionary<string, string?>>(JsonConvert.SerializeObject(availableRoomsQuery)) ?? throw new NullException("query");
